...
- When `LANGSMITH_TRACING=true`, traces default to the shared projects `email-assistant:agent` and `email-assistant:judge`. Override the assistant grouping with `EMAIL_ASSISTANT_TRACE_PROJECT` and judge grouping with `EMAIL_ASSISTANT_JUDGE_PROJECT` (or `EMAIL_ASSISTANT_JUDGE_PROJECT_OVERRIDE`).
