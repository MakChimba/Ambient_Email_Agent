...
## Planned / Upcoming
- Early Oct — Harden eval-mode control flow (`llm_call` short-circuit, trimmed history in `should_continue` / `mark_as_read`) and re-run `EMAIL_ASSISTANT_EVAL_MODE=1 … pytest tests/test_response.py --agent-module=email_assistant_hitl_memory_gmail -k "tool_calls and email_input1"`. Full `-k tool_calls` sweep remains pending local timeout mitigation.
- Early Oct — Address PR30 follow-ups: Gmail tool logging/simulation guardrails, forced HITL confirmation for `mark_as_spam_tool`, reminder dispatcher durability refresh, and `interrupt_handler` ↔ `mark_as_read_node` edge restoration. Latest offline HITL smoke traces in `logs/email_assistant.log` stay as reference until validation finishes.
- Early Oct — Bootstrap local venv for pytest tooling, harden `interrupt_handler_task` against ToolMessage-only histories, and rerun targeted pytest command with `HITL_AUTO_ACCEPT=1 EMAIL_ASSISTANT_SKIP_MARK_AS_READ=1 EMAIL_ASSISTANT_EVAL_MODE=1` once preceding fixes land.
- 2025-09-30 — LangGraph CLI compatibility: guarded all graph `compile()` calls and the SQLite store helper behind `LANGGRAPH_DISABLE_CUSTOM_CHECKPOINTER` so `langgraph dev`/Cloud runs rely on platform persistence while local pytest retains the SQLite defaults. README_LOCAL now documents the env flag for Agent Inbox testing.
